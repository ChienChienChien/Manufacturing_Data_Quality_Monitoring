...
這套機制無法取代所有業務邏輯驗證，但能為資料是否適合進入決策流程建立一致且可追蹤的基本門檻，降低最低成本 BOM、庫存推估與缺料判斷使用異常資料的風險。

### 2. 將資料檢核轉為每日自動化流程

檢核程式部署於 Windows Server，由 Windows 工作排程器於每天上午 8 點自動觸發。Python 透過 Great Expectations 檢核資料品質規則，並將每次檢核的整體狀態與檢核項目明細結果寫入 SQL Server，再同步更新 Power BI 監控畫面。
